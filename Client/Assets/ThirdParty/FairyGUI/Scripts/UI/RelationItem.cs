...
namespace FairyGUI
{
    ...
    class RelationItem
    {
        ...
        /// <summary>
        /// 检测是否有 Width_Width 和 Height_Height的关联类型
        /// </summary>
        public bool CheckIsHasWWAndHHRelationType()
        {
            bool isContainsWW = false;
            bool isContainsHH = false;
            for (int i = 0; i < _defs.Count; i++)
            {
                if (_defs[i].type == RelationType.Width)
                    isContainsWW = true;
                
                if (_defs[i].type == RelationType.Height)
                    isContainsHH = true;
                if (isContainsWW && isContainsHH)
                    return true;
            }
