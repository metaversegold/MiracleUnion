...
Shader "Custom/ViewSpecular" {
	...
	SubShader {
		...
		Pass {
			...
			{
				fixed4 color = tex2D(_MainTex, i.uv.xy) * _Tint;
                clip(color.a - _Cut);
				...
				fixed3 specColor = _Spec.rgb * 4 * (specular * specular * specular * specular * specular) * _Spec.a;
				return fixed4(color.rgb + specColor, color.a);
            }
            ENDCG
        }
	} 
