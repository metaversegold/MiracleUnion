...
Shader "Custom/MuCharacter-3UVLaye-2Specularr-Alpha" {
	Properties {
		...
		Pass
		{
			...
			half4 frag(VS_Output i) : COLOR
			{
				...
				half3x3 matView2World = half3x3(rightView,
														upView,
														forwardView);
				half3 viewDirInViewSpace = tex2D(_ViewDirTex2, float2(_Time.x * _TimeScale2, 0)).xyz * 2 - 1;
				half3 viewDirW = mul(matView2World, viewDirInViewSpace);

				half reflection = max( 0, dot(normalize(viewDirW), normalize(i.normalW)) );
				half specPow = saturate(reflection * _LightScale - _LightScale + 1);
				half4 layerColor2 =  _SpecColor2 * specPow;

				return baseColor + baseColor.a*layerColor2;
			}
